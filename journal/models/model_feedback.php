<?php
	class Model_Feedback extends Model
	{
		public function send_data($data = null)
		{
			...
			if (!empty($data)) {
				$result = $this->executeQuery("INSERT INTO feedback (user, comment, email) VALUES ('".$data['user']."', '".$data['comment']."',
				'".$data['email']."');");
				//Если запрос пошел не так получаем код ошибки и отдаем в качестве ответа на запрос
				if (!$result) {
